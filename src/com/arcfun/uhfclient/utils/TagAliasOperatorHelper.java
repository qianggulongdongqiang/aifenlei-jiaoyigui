...
public class TagAliasOperatorHelper {
    ...
    public void handleAction(Context context, int sequence, String mobileNumber) {
        ...
        LogUtils.d(TAG, "sequence:" + sequence + ",mobileNumber:"
                + mobileNumber);
     ...
     * 处理设置tag
     * */
    public void handleAction(Context context, int sequence,
    ...
    private boolean RetryActionIfNeeded(int errorCode, TagAliasBean tagAliasBean) {
        ...
        if (errorCode == 6002 || errorCode == 6014) {
            ...
            if (tagAliasBean != null) {
                ...
                String logs = getRetryStr(tagAliasBean.isAliasAction,
                        tagAliasBean.action, errorCode);
                Utils.showMsg(context, logs);
                return true;
            }
        }
        return false;
    ...
    private boolean RetrySetMObileNumberActionIfNeeded(int errorCode,
            String mobileNumber) {
        if (!Utils.isConnected(context)) {
            LogUtils.w(TAG, "no network");
            return false;
    ...
    private String getRetryStr(boolean isAliasAction, int actionType,
            int errorCode) {
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        LogUtils.i(TAG, "action - onTagOperatorResult, sequence:" + sequence
        ...
        if (tagAliasBean == null) {
            Utils.showMsg(context, "获取缓存记录失败");
            return;
        }
    ...
    public void onCheckTagOperatorResult(Context context,
            JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        LogUtils.i(TAG, "action - onCheckTagOperatorResult, sequence:"
                + sequence + ",checktag:" + jPushMessage.getCheckTag());
        init(context);
        // 根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = (TagAliasBean) setActionCache.get(sequence);
        if (tagAliasBean == null) {
        ...
        if (jPushMessage.getErrorCode() == 0) {
            ...
            setActionCache.remove(sequence);
            String logs = getActionStr(tagAliasBean.action) + " tag "
                    + jPushMessage.getCheckTag() + " bind state success,state:"
                    + jPushMessage.getTagCheckStateResult();
            LogUtils.i(TAG, logs);
            Utils.showMsg(context, logs);
        } else {
            String logs = "Failed to " + getActionStr(tagAliasBean.action)
                    + " tags, errorCode:" + jPushMessage.getErrorCode();
            LogUtils.e(TAG, logs);
            if (!RetryActionIfNeeded(jPushMessage.getErrorCode(), tagAliasBean)) {
                Utils.showMsg(context, logs);
            }
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        LogUtils.i(TAG, "action - onAliasOperatorResult, sequence:" + sequence
        ...
        if (jPushMessage.getErrorCode() == 0) {
            LogUtils.i(TAG, "action - modify alias Success,sequence:"
                    + sequence);
            setActionCache.remove(sequence);
    ...
    // 设置手机号码回调
    public void onMobileNumberOperatorResult(Context context,
            JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        LogUtils.i(TAG, "action - onMobileNumberOperatorResult, sequence:"
                + sequence + ",mobileNumber:" + jPushMessage.getMobileNumber());
        init(context);
        if (jPushMessage.getErrorCode() == 0) {
            LogUtils.i(TAG, "action - set mobile number Success,sequence:"
                    + sequence);
            setActionCache.remove(sequence);
        } else {
            String logs = "Failed to set mobile number, errorCode:"
                    + jPushMessage.getErrorCode();
            LogUtils.e(TAG, logs);
            if (!RetrySetMObileNumberActionIfNeeded(
                    jPushMessage.getErrorCode(), jPushMessage.getMobileNumber())) {
                Utils.showMsg(context, logs);
            }
        }
    }
